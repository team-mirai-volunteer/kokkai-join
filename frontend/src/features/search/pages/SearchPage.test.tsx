...
describe("SearchPage", () => {
  ...
  it("should display synthesis text in search result area during synthesis", async () => {
    const user = userEvent.setup();

    // Mock streaming search with synthesis_chunk events
    const progressEvents: ProgressEvent[] = [
      {
        type: "progress",
        step: 5,
    ...
    // Check that synthesis text appears in search result area
    await waitFor(
      () => {
    ...
    // Progress display should be hidden when synthesisText exists
    expect(screen.queryByRole("progressbar")).not.toBeInTheDocument();
  });

  it("should display streaming markdown chunks progressively using real chunks.txt data", async () => {
    const user = userEvent.setup();

    // Load real chunks from fixtures
    const chunksPath = join(process.cwd(), "fixtures", "chunks.txt");
    const chunksContent = readFileSync(chunksPath, "utf-8");

    // Parse SSE events from chunks.txt
    const lines = chunksContent.split("\n");
    const events: string[] = [];
    let currentEvent = "";
    ...
    // Create a stream from the parsed events
    const encoder = new TextEncoder();
    const stream = new ReadableStream({
      async start(controller) {
        for (const event of events) {
    ...
    const input = screen.getByPlaceholderText(/検索キーワードを入力/);
    await user.type(input, "政治資金");

    const submitButton = screen.getByRole("button", { name: "検索" });
    await user.click(submitButton);

    // Wait for first synthesis chunk to appear
    await waitFor(
      () => {
        const resultDiv = container.querySelector(".markdown-output");
        expect(resultDiv).toBeInTheDocument();
        // First character from chunks.txt is "#"
        expect(resultDiv?.textContent).toMatch(/政/);
      },
      { timeout: 5000 },
    );

    // Check that text is accumulating progressively
    await waitFor(
      () => {
        const resultDiv = container.querySelector(".markdown-output");
        expect(resultDiv?.textContent).toContain("政治資金");
      },
      { timeout: 5000 },
    );
    ...
    await waitFor(
      () => {
        const resultDiv = container.querySelector(".markdown-output");
        expect(resultDiv?.textContent?.length).toBeGreaterThan(100);
      },
      { timeout: 10000 },
    );

    // Progress bar should be hidden when synthesisText exists
    expect(screen.queryByRole("progressbar")).not.toBeInTheDocument();
  });
});
