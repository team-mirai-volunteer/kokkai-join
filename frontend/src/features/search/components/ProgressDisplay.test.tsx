...
describe("ProgressDisplay", () => {
  ...
  it("should not display message when not provided", () => {
    ...
    expect(progressDisplay?.textContent).toBe(
      "ステップ 1/5: クエリプランニング",
    );
  });
  ...
  it("should display synthesis step with progress bars", () => {
    const progress: ProgressState = {
      step: 5,
      totalSteps: 5,
      stepName: "セクション統合",
    ...
    // 進捗情報は表示される
    expect(
      screen.getByText("ステップ 5/5: セクション統合"),
    ).toBeInTheDocument();
    expect(screen.getByRole("progressbar")).toBeInTheDocument();
    expect(screen.getByText("AI回答を生成しています...")).toBeInTheDocument();
  });
});
