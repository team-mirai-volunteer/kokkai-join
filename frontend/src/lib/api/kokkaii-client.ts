...
export class KokkaiiAPIClient {
      ...
      for (let monthsBack = 0; monthsBack < maxMonthsToCheck; monthsBack++) {
        ...
        try {
          ...
          if (response.meetingRecord && response.meetingRecord.length > 0) {
            const transformedMeetings = this.transformToRecentMeetings(response.meetingRecord);
  ...
  async getMeetingDetail(issueId: string): Promise<MeetingRecord | null> {
    try {
      const response = await fetch(
        `${this.baseURL}/meeting?issueID=${encodeURIComponent(issueId)}&recordPacking=json`,
        {
          next: {
  ...
  private transformToRecentMeetings(meetings: MeetingRecord[]): RecentMeeting[] {
