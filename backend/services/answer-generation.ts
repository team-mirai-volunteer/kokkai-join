...
import { CEREBRAS_MODEL, cerebrasClient } from "../config/cerebras.ts";
import type {
	MidSummary,
	SpeechResult,
	SubSummary,
...
export class AnswerGenerationService {
	...
	private async consolidateIntermediate(
		...
		const input = allSummaries
			.map(
				(s) =>
					`- speechId: ${s.speechId}, viewpoint: ${s.viewpoint}, content: ${s.content}`,
			)
			.join("\n");
		...
		for (const mid of midSummaries) {
			...
			for (const speechId of mid.speechIds) {
				...
				if (speech) {
					...
					markdown += `| ${speech.speaker} | ${speech.party} | ${formattedDate} | ${contentPreview} | ${speech.url} |\n`;
				}
			}
			markdown += `\n---\n\n`;
		}
