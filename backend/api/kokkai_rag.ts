...
import { DEFAULT_OLLAMA_BASE_URL, EMBEDDING_MODEL_NAME, MAX_DB_CONNECTIONS, DEFAULT_TOP_K_RESULTS } from "../config/constants.ts";
...
interface RagSearchRequest { query: string; limit?: number }
interface RagSearchResponse { results: DocumentResult[]; meta: { total: number; tookMs: number; timestamp: string } }
...
class KokkaiRagApi {
  ...
  private setupMiddleware() {
    this.app.use("*", cors({ origin: "*", allowMethods: ["GET","POST"], allowHeaders: ["Content-Type"] }));
    this.app.use("*", logger());
    this.app.use("*", prettyJSON());
  }

  private setupRoutes() {
    this.app.get("/v1/health", (c) => c.json({ status: "ok", timestamp: new Date().toISOString() }));
      ...
      async (c) => {
        ...
        try {
          ...
          const resp: RagSearchResponse = {
            results: docs,
            meta: { total: docs.length, tookMs: Date.now() - start, timestamp: new Date().toISOString() },
          };
          return c.json(resp);
        } catch (e) {
    ...
    this.app.onError((err, c) => c.json({ error: "Internal Server Error", message: err.message }, 500));
  }
  ...
  async initialize(): Promise<void> {
    ...
    Settings.embedModel = new OllamaEmbedding({ model: EMBEDDING_MODEL_NAME, config: { host: ollamaBaseUrl } });

    this.dbPool = new Pool({ connectionString: databaseUrl, max: MAX_DB_CONNECTIONS });
    const client = await this.dbPool.connect();
    try { await pgvector.registerTypes(client); } finally { client.release(); }
