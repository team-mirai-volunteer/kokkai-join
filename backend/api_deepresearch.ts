	...
	private setupRoutes(): void {
		...
		this.app.post(
			...
			validator("json", (value, c) => {
				const result = v.safeParse(DeepResearchRequestSchema, value);
				if (!result.success) {
					const errors = v.flatten(result.issues);
					const firstError =
						Object.values(errors.nested || {}).flat()[0] || "Invalid request";
			...
			async (c) => {
				...
				try {
					const req = await c.req.json<DeepResearchRequestValidated>();
